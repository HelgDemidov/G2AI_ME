...
4. WebFetch is allowed for METADATA pages (landing pages, press releases) only. **Never WebFetch the document body** — it passes through a small model and is not verbatim (CLAUDE.md rule); body acquisition belongs to `run_pipeline.py`, after triage.
...
- Do NOT download or WebFetch document bodies (acquisition is `run_pipeline.py`, post-triage).
