...
# 4. ⚠ ТРЕБУЕТСЯ rclone >= 1.74. На 1.60 (версия из apt в Ubuntu 24.04) каждый файл
#    даёт ложную ошибку: R2 возвращает на успешный PUT заголовок `x-amz-version-id`,
#    rclone 1.60 использует его в проверочном `HEAD ?versionId=...`, а чтение по версии
#    R2 не реализует -> 501 Not Implemented -> передача считается неудавшейся и
#    ретраится целиком. Файлы при этом заливаются (со второй попытки) — симптом
#    именно такой: шум из ошибок при формально успешном результате. В 1.74 починено
#    (проверено живьём 2026-07-26). Если вдруг окажетесь на старой версии — лечится
#    добавлением `RCLONE_CONFIG_R2_NO_HEAD=true`, но правильнее обновить rclone:
#    apt отдаёт только 1.60, ставить с `curl https://rclone.org/install.sh | sudo bash`.
#    Сверить копию можно в любой момент:
