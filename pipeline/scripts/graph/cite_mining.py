...
IDENTIFIER_PREFIXES = ("CELEX:", "SLCG:", "ISO", "NIST ")
"""Префиксы канонических идентификаторов, эмитируемых канонизаторами ``_PATTERNS``
выше — единственное определение (knowledge-hardening §4). Новая семья паттернов
добавляется ЗАПИСЬЮ в ``_PATTERNS`` и, если у неё новый префикс, — записью здесь: одно
место вместо рассинхронизируемой копии (раньше — рукописный кортеж в тесте алиасов)."""
...
# Хвосты: `-YYYYMMDD` — консолидированная версия, `R(NN)`/`(NN)` — corrigendum и
# порядковый номер документа В ВЫПУСКЕ OJ; все входят в идентификатор, иначе поправка
# резолвилась бы в базовый акт (неверная связь).
# ⚠ Литера `R` необязательна (spec triage-intake-hardening §5, 2026-07-28): форма БЕЗ
# неё (`CELEX:52026M12391(01)` — обычная порядковая позиция, не corrigendum) раньше не
# захватывалась, совпадение обрывалось на `52026M12391`, то есть цитата резолвилась бы
# в ДРУГОЙ, реально существующий документ — ровно тот класс неверной связи, который
# строка выше объявляет закрытым. Замер на слое кандидатов: извлекаемых id 287 -> 305,
# групп коллизии 19 -> 1 (оставшаяся — настоящий кросс-коннекторный дубль).
