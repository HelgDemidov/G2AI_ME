...
def run_tesseract(raw: Path, workdir: Path) -> dict[int, str] | None:
    ...
    ``None``, если у документа нет текст-слоя (``pdfmeta.was_ocr_normalized`` вернул
    ``False``) — кандидат пропускается с сообщением, не роняет прогон (spec §3)."""
    copy = _copy_raw(raw, workdir)
    if not pdfmeta.was_ocr_normalized(copy):
        return None
...
def _tesseract_result(raw: Path, workdir: Path, pages: list[int]) -> CandidateResult:
    ...
    if pages_map is None:
        return CandidateResult(
            ...
            failed="raw не нормализован OCR (нет текст-слоя, pdfmeta.was_ocr_normalized=False)",
        )
