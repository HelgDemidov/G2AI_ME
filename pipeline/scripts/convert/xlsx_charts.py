...
def _anchor_cell(anchor: Any) -> str:
    ...
    col = int(frm.findtext(_q("xdr", "col")))
    row = int(frm.findtext(_q("xdr", "row")))
...
def extract_chart_workbook(raw: Path, id12: str) -> bytes | None:
    ...
    ``id12`` (см. ``figures_vlm._render_xlsx_chart``): ВЕСЬ zip оригинала
    наследуется без изменений (серии чарта ссылаются на диапазоны ячеек по
    формуле — данные должны остаться разрешимыми), кроме (а) лист-хозяин
    форсированно visible, все остальные листы скрыты (LibreOffice PDF-экспорт
    по умолчанию рендерит только видимые листы — иначе на выходе страница на
    КАЖДЫЙ лист книги, а не одна для чарта; форсированный visible покрывает и
    случай, когда сам чарт живёт на исходно скрытом листе); (б) drawing-парт
    листа-хозяина обрезан до ОДНОГО целевого чарта (иначе кроп заденет
    соседний чарт/фигуру того же drawing-парта). None — чарт с таким id12 не
    найден при пере-детекции (raw изменился?)."""
    with zipfile.ZipFile(raw) as z:
        names = z.namelist()
        name_set = set(names)
        target: tuple[str, str, Any] | None = None  # (host_sheet, drawing_part, keep_anchor)
        ...
        for sheet_name, sheet_part in _sheet_parts(z).items():
            ...
            for anchor, chart_ref in _chart_anchors(drawing_root):
                ...
                if hashlib.sha256(etree.tostring(chart_root)).hexdigest()[:12] == id12:
                    target = (sheet_name, drawing_part, anchor)
                    break
            if target is not None:
                break
        if target is None:
            return None
        host_sheet, drawing_part, keep_anchor = target
        ...
        for sheet_el in wb_root.findall(f".//{_q('main', 'sheet')}"):
            sheet_el.set("state", "visible" if sheet_el.get("name") == host_sheet else "hidden")
